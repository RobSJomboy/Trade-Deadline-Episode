...
1. **Single reveal, not a draft board** — search a player, search a team, hit Show on each. Same 750ms ease, headshot → red arrow → team logo layout, sitting low on the frame just above the ticker.
   - **Hiding drops the graphic behind the ticker** over 750ms. `#tradeRow` is clipped at `y=980` (the ticker's bottom edge) and the bar paints after it, so the group slides down, gets occluded by the bar, and is gone — rather than dissolving in mid-air. Opacity deliberately holds for the first 490ms and fades only over the last 260ms (490 + 260 = 750), so you see it travel *under* the bar instead of vanishing before it gets there.
   - Enter and exit use **separate transitions**: the entrance keeps its original ease-out rise, while the exit uses an accelerating `cubic-bezier(.45,0,.85,.35)` so it reads as dropping away. Because a CSS transition's timing comes from the destination state, the entrance curve lives on the `.show` rules and the exit curve on the base rules — **edit the right one**.
   - Exit travel distances are sized to clear that clip line, not picked for looks: images 650px, arrow 320px (needs >269), name tags 160px (needs >101). The arrow previously only scaled horizontally and the tags only moved 20px, so neither could reach the bar at all.
