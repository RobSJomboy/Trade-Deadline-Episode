...
1. **Single reveal, not a draft board** — search a player, search a team, hit Show on each. Same 750ms ease, headshot → red arrow → team logo layout.
2. **Trade Tracker ticker** (bottom of screen) — white background, `#173a56` text, continuous auto-scrolling crawl.
...
4. **Logo bug** — small Talkin' Baseball corner watermark, toggleable, baked into the display file.
...
- The Passan alert sound and the logo bug are embedded directly in `display.html` (base64), so there's no separate audio or image file to keep track of or accidentally leave out of a repo push.
